...
# 	if(trait=="weight_g"){
# 		stan_data_trait <- stan_data_ped_noRep
# 		stanModel_pedN <- cmdstan_model(paste0(wd,"stan/normal_RE_day15_ped1.stan"))
# 	}else{
# 		stan_data_trait <- c(list(y=THBW[,trait]),stan_data_ped)
# 		stanModel_pedN <- cmdstan_model(paste0(wd,"stan/normal_RE_day15_ME_ped1.stan"))
# 	}
...
# 	save(model_z, file= paste0(wd,"Data/Intermediate/stanModNormal_pedN_",if(reduced)"reduced_",trait,"_",format(Sys.time(), "%Y%m%d_%H%M"),".Rdata"))
