...
# head(gammasDS_alpha10)
# par(mfrow=c(2,1))
# hist(gammasDS_alpha10[,"gamma_dam_sire"], breaks=100)
# hist(gammasDS_delta[,"gamma_dam_sire"], breaks=100)
...
{
	par(mfrow=c(3,2), mar=c(3,4,5,1))

	pp_plot(x = seq(-1,1,length.out=101), 
		prior = dunif(seq(-1,1,length.out=101), -1,1),
		posterior=model_zpostDS_delta[,"delta_dam_sire"],main="delta_dam_sire"
	)

	pp_plot(x = gamma_prior1$x, 
		prior = gamma_prior1$y,
		posterior=model_zpostDS_delta[,"gamma_dam_sire"], main="gamma_dam_sire"
	)


	pp_plot(x = seq(-40,40,length.out=101), 
		prior = dnorm(seq(-40,40,length.out=101), 0,10),
		posterior=model_zpostDS_alpha10[,"alpha_dam_sire"],main="alpha_dam_sire"
	)
	pp_plot(x = gamma_prior2$x, 
		prior = gamma_prior2$y,
		posterior=model_zpostDS_alpha10[,"gamma_dam_sire"], main="gamma_dam_sire"
	)


	pp_plot(x = seq(-5,5,length.out=101), 
		prior = dnorm(seq(-5,5,length.out=101), 0,1),
		posterior=model_zpostDS_alpha1[,"alpha_dam_sire"],main="alpha_dam_sire"
	)
	pp_plot(x = gamma_prior3$x, 
		prior = gamma_prior3$y,
		posterior=model_zpostDS_alpha1[,"gamma_dam_sire"], main="gamma_dam_sire"
	)
}
