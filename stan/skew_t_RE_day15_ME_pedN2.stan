...
parameters {
  ...
  // real<lower=log(4)> log_nu_nest;

  vector [N_ind] ind_effects;
  real<lower=0> sigma_ind;
  real alpha_ind;  
  real<lower=4> nu_ind;
  // real<lower=log(4)> log_nu_ind;
...
transformed parameters{
  ...
  // real nu_ind= exp(log_nu_ind);
  // real nu_nest= exp(log_nu_nest);
...
model {
  ...
  alpha_nest ~ normal(0,1);
  sigma_nest ~ cauchy(0,10);
  nu_nest ~ uniform(4,40);
  // log_nu_nest ~ uniform(log(4),log(40));
  
  alpha_ind ~ normal(0,1);
  sigma_ind ~ cauchy(0,10);
  nu_ind ~ uniform(4,40);  
  // log_nu_ind ~ uniform(log(4),log(40));
  ...
  ind_scaled = (ind_effects-xi_ind)/omega_ind * alpha_ind;
